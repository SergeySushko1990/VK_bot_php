...
function vkApi_usersGet($user_id) {
  return vkApi_call('users.get', array(
    'user_id' => $user_id,
  ));
}

function vkApi_photosGetMessagesUploadServer($peer_id) {
  return vkApi_call('photos.getMessagesUploadServer', array(
    'peer_id' => $peer_id,
  ));
}

function vkApi_photosSaveMessagesPhoto($photo, $server, $hash) {
  return vkApi_call('photos.saveMessagesPhoto', array(
    'photo'  => $photo,
    'server' => $server,
    'hash'   => $hash,
  ));
...
function vkApi_docsGetMessagesUploadServer($peer_id, $type) {
  return vkApi_call('docs.getMessagesUploadServer', array(
    'peer_id' => $peer_id,
    'type'    => $type,
  ));
}
...
function vkApi_docsSave($file, $title) {
  return vkApi_call('docs.save', array(
    'file'  => $file,
    'title' => $title,
  ));
}
